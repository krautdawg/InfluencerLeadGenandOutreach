...
async def enrich_profile_batch(usernames, ig_sessionid, apify_token, perplexity_key, semaphore):
    ...
    async with semaphore:
        try:
            ...
            profile_data = await call_apify_actor("8WEn9FvZnhE7lM3oA", input_data, apify_token)
            enriched_profiles = []
            
            # Process each profile
            for username in usernames:
                profile_info = profile_data.get('items', [{}])[0] if profile_data.get('items') else {}
                
                # Check if contact info is missing and use Perplexity fallback
                if not any([profile_info.get('email'), profile_info.get('phone'), profile_info.get('website')]):
                    try:
                ...
                enriched_profiles.append({
                    ...
                    'followersCount': profile_info.get('followersCount', 0),
                    'followingCount': profile_info.get('followingCount', 0),
                    'postsCount': profile_info.get('postsCount', 0),
...
def index():
    """Main page"""
    ig_sessionid = session.get('ig_sessionid') or os.environ.get('IG_SESSIONID')
...
async def process_keyword_async(keyword, ig_sessionid):
    ...
    perplexity_semaphore = asyncio.Semaphore(5)  # Limit to 5 concurrent Perplexity calls
    ...
    for batch in batches:
        task = enrich_profile_batch(batch, ig_sessionid, apify_token, perplexity_key, semaphore)
        tasks.append(task)
...
def draft_email(username):
    ...
    data = request.get_json()
    subject_prompt = data.get('subject_prompt', 'Generate a compelling subject line for an outreach email')
    body_prompt = data.get('body_prompt', 'Generate a personalized outreach email')
    
    # Find the lead
    lead = next((l for l in app_data['leads'] if l['username'] == username), None)
    if not lead:
    ...
    try:
        ...
        subject_response = openai_client.chat.completions.create(
            ...
            messages=[
                ...
                {"role": "user", "content": f"Profile: @{lead['username']}, Name: {lead['fullName']}, Bio: {lead['bio']}"}
            ],
        ...
        body_response = openai_client.chat.completions.create(
            model="gpt-4o",
            messages=[
                {"role": "system", "content": f"{body_prompt} Respond with JSON format: {{\"body\": \"email content\"}}"},
                {"role": "user", "content": f"Profile: @{lead['username']}, Name: {lead['fullName']}, Bio: {lead['bio']}, Email: {lead['email']}"}
            ],
            response_format={"type": "json_object"},
            max_tokens=500
        )
        ...
        lead['subject'] = subject_data.get('subject', 'Collaboration Opportunity')
        lead['emailBody'] = body_data.get('body', 'Hello, I would like to discuss a collaboration opportunity.')
    ...
    try:
        if format == 'csv':
            ...
            writer = csv.DictWriter(output, fieldnames=[
                'username', 'fullName', 'bio', 'email', 'phone', 'website',
            ...
            return {
                "data": json.dumps(app_data['leads'], indent=2),
