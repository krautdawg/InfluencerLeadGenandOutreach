...
with app.app_context():
    ...
    def initialize_variable_settings():
        ...
        try:
            ...
            db.session.commit()
            logger.info("Variable settings initialized successfully")
        except Exception as e:
            logger.error(f"Failed to initialize variable settings: {e}")
            db.session.rollback()

    initialize_variable_settings()
...
def build_dynamic_prompt_content(lead, prompt_type, has_product):
    ...
    try:
        ...
        # Return empty string on error - no fallbacks
        return ""
...
def draft_email(username):
    ...
    try:
        ...
        logger.info(f"Generated content for {username}: subject={len(subject_text)} chars, body={len(body_text)} chars")

        # Update lead
        lead.subject = subject_text
        ...
        logger.error(f"Email generation failed for {username}: {str(e)}", exc_info=True)
        return {"error": f"Failed to generate email: {str(e)}"}, 500
